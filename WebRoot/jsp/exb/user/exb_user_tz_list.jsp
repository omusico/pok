...
<html>
<head>
<title>管理平台-快乐十分 用户投注信息显示</title>
<style type="text/css">
...
<!--
	function to_del(tzId,userId){
		if(confirm("删除后将无法恢复，确实要删除吗?")){
 		...
 		var url = "/exb/exbPublicInfo.do?method=delUserTzInfo&tzId="+tzId+"&userId=<%=userId%>";
...
<tr>
<td>
		<display:table name="tzInfoList" cellspacing="0" cellpadding="0"   pagesize="10" class="table"  requestURI="/exb/exbPublicInfo.do?method=viewUserTzInfo"  decorator="com.onmyway.common.displaytag.DisplayTagWrapper">
		...
		%>
			<display:column property="exbUserTouzhuHref" title="操作" />
		<%
			}
		%>
		</display:table>
