...
//选1：数投
function randSelectShutou(num){
	randSelect(1,18,num);
}
//选1：红投
function randSelectHongtou(num){
	randSelect(19,20,num);
}


//num:需要机选的注数
function randSelect(beginNum,endNum,num)
{
	for(var i=0;i<num;i++)
	{
		var textRanTemp = randSelectOneCom(beginNum,endNum);
...
function randSelectOneCom(beginNum,endNum){
  var arrRanNumFig=new Array();
  for(var i=0;i<1;i++){
    arrRanNumFig[i]=generRandomNum(beginNum,endNum);
  }
