...
<SCRIPT LANGUAGE="JavaScript">
<!--
	function to_del(tzId){
	    if(confirm("删除后将无法恢复，确实要删除吗?")){
 		  var url = "/ssc/sscPublicInfo.do?method=delUserTzInfo&tzId="+tzId+"&userId=<%=userId%>";
...
<tr>
<td>
		<display:table name="tzInfoList" cellspacing="0" cellpadding="0"   pagesize="10" class="table"  requestURI="/ssc/sscPublicInfo.do?method=viewUserTzInfo"  decorator="com.onmyway.common.displaytag.DisplayTagWrapper">
		...
		%>
			...
			<display:column property="sscUserTouzhuHref" title="操作" />
	   <%
			}
		%>
		</display:table>
