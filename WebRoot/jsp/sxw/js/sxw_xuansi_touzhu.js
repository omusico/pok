...
function wagerPlayCalEach(){
  ...
  if(tempLen < 4){
	...
	totalQuan=arrQuanForPlay[0]*(arrQuanForPlay[0]-1)*(arrQuanForPlay[0]-2)*(arrQuanForPlay[0]-3)/24;
  }
...
//	alert("one="+rowOneLen + ",two="+rowTwoLen);
	...
	var totalQuan = 0;
	if((rowOneLen<1 || rowTwoLen <1) && (rowOneLen+rowTwoLen < 4) ){
		totalQuan=0;
	}else{
		if(rowOneLen == 1){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2)/6;//如果胆码选择的是1个，则投注注数为拖胆中任意3个的组合
		}else if(rowOneLen == 2){
			totalQuan = rowTwoLen * (rowTwoLen-1)/2;//如果胆码选择的是2个，则投注注数为拖胆中任意2个的组合
		}else if(rowOneLen == 3){
			totalQuan = rowTwoLen;//如果胆码选择的是3个，则投注注数为拖胆的个数
		}
	}
