    ...
    if(framTime.document.readyState=='complete'){
	    if(framTime.document.all("remainsecond").innerHTML==""){
	      ...
	      // if(framTime.document.all("remainsecond").innerHTML=="00:30"){
			...
			//得到开奖信息
			DwrBean.getSxwCurrentZjNum(zjNumCallBack);
						
			//刷新首页的开奖列表
			parent.document.frames['frawinnumshow'].location.href = "/sxw/sxwPublicInfo.do?method=showSxwZjNum&indexFlag=indexPage";
...
function zjNumCallBack(obj){
	...
	$("#divWan").attr("innerHTML",obj.wan <10 ? ("0"+obj.wan) : obj.wan);
	$("#divQian").attr("innerHTML",obj.qian <10 ? ("0"+obj.qian) : obj.qian);
	$("#divBai").attr("innerHTML",obj.bai <10 ? ("0"+obj.bai) : obj.bai);
	$("#divShi").attr("innerHTML",obj.shi <10 ? ("0"+obj.shi) : obj.shi);
	$("#divGe").attr("innerHTML",obj.ge <10 ? ("0"+obj.ge) : obj.ge);
}
/*
