...
function wagerPlus(){
  ...
  //alert(playMode);
  if(playMode == "zhiqian2"){//如果是直选前2，则需要对十位及个位数进行计算
...
///-------------------选前2直选----------------------------------
//选前2直选
function chooseZhiqian2Play(weizhi,numb){
	...
	//将不同数位上相同的数置为未选中
	 ...
	 for(i=0;i<objTempWeizhi.length;i++){
	  ...
	  zhiqian2wagerPlayGetText(arPlTy);
	  xuan2zhiqian2Cal();
}

//初始化数组
function zhiqian2wagerPlayGetText(arrPlayType){//公共com,gr6
