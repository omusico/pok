...
function wagerPlayCalEach(){
  ...
  if(tempLen < 7){
	...
	totalQuan=arrQuanForPlay[0]*(arrQuanForPlay[0]-1)*(arrQuanForPlay[0]-2)*(arrQuanForPlay[0]-3)*(arrQuanForPlay[0]-4)*(arrQuanForPlay[0]-5)*(arrQuanForPlay[0]-6)/5040;
  }
...
//	alert("one="+rowOneLen + ",two="+rowTwoLen);
	...
	var totalQuan = 0;
	if((rowOneLen<1 || rowTwoLen <1) && (rowOneLen+rowTwoLen < 6) ){
		totalQuan=0;
	}else{
		if(rowOneLen == 1){
