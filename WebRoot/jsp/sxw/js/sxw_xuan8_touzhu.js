...
function wagerPlayCalEach(){
  ...
  if(tempLen < 8){
	...
	totalQuan=arrQuanForPlay[0]*(arrQuanForPlay[0]-1)*(arrQuanForPlay[0]-2)*(arrQuanForPlay[0]-3)*(arrQuanForPlay[0]-4)*(arrQuanForPlay[0]-5)*(arrQuanForPlay[0]-6)*(arrQuanForPlay[0]-7)/40320;
  }
...
function wagerPlus(){
  ...
  if(playMode.value=="fushi"){
   ...
   var textCombined=arrTextForPlay[0]; 
...
//	alert("one="+rowOneLen + ",two="+rowTwoLen);
	...
	var totalQuan = 0;
	if((rowOneLen<1 || rowTwoLen <1) && (rowOneLen+rowTwoLen < 7) ){
		totalQuan=0;
	}else{
		 
		if(rowOneLen == 1){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2) * (rowTwoLen-3) * (rowTwoLen-4) * (rowTwoLen-5)* (rowTwoLen-6)/5040;//如果胆码选择的是1个，则投注注数为拖胆中任意7个的组合
		}else if(rowOneLen == 2){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2) * (rowTwoLen-3) * (rowTwoLen-4) * (rowTwoLen-5)/720;//如果胆码选择的是1个，则投注注数为拖胆中任意6个的组合
		}else if(rowOneLen == 3){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2) * (rowTwoLen-3) * (rowTwoLen-4) /120;//如果胆码选择的是2个，则投注注数为拖胆中任意5个的组合
		}else if(rowOneLen == 4){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2) * (rowTwoLen-3) /24;//如果胆码选择的是3个，则投注注数为拖胆中任意4个的组合
		}else if(rowOneLen == 5){
			totalQuan = rowTwoLen* (rowTwoLen-1) * (rowTwoLen-2) /6;//如果胆码选择的是4个，则投注注数为拖胆任意3个的组合
		}else if(rowOneLen == 6){
			totalQuan = rowTwoLen* (rowTwoLen-1)  /2;//如果胆码选择的是4个，则投注注数为拖胆任意2个的组合
		}else if(rowOneLen == 7){
