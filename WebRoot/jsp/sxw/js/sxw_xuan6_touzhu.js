...
function wagerPlayCalEach(){
  ...
  if(tempLen < 6){
	...
	totalQuan=arrQuanForPlay[0]*(arrQuanForPlay[0]-1)*(arrQuanForPlay[0]-2)*(arrQuanForPlay[0]-3)*(arrQuanForPlay[0]-4)*(arrQuanForPlay[0]-5)/720;
  }
...
//	alert("one="+rowOneLen + ",two="+rowTwoLen);
	...
	if((rowOneLen<1 || rowTwoLen <1) && (rowOneLen+rowTwoLen < 6) ){
		...
		if(rowOneLen == 1){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2) * (rowTwoLen-3) * (rowTwoLen-4)/120;//如果胆码选择的是1个，则投注注数为拖胆中任意5个的组合
		}else if(rowOneLen == 2){
			totalQuan = rowTwoLen * (rowTwoLen-1) * (rowTwoLen-2)* (rowTwoLen-3)/24;//如果胆码选择的是2个，则投注注数为拖胆中任意4个的组合
		}else if(rowOneLen == 3){
			totalQuan = rowTwoLen * (rowTwoLen-1)* (rowTwoLen-2)/6;//如果胆码选择的是3个，则投注注数为拖胆中任意3个的组合
		}else if(rowOneLen == 4){
			totalQuan = rowTwoLen * (rowTwoLen-1)/2;//如果胆码选择的是4个，则投注注数为拖胆中任意2个的数
		}else if(rowOneLen == 5){
			totalQuan = rowTwoLen;//如果胆码选择的是4个，则投注注数为拖胆中数
		}
	}
