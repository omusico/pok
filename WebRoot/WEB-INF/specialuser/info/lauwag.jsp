	        ...
	        <table>
			  ...
			  <td>
			   <table align=center width=910 height=90 border=1 cellpadding=0 cellspacing=0 bordercolor=pink>
			    <tr><td colspan=10 height=30 align=center>时时乐 投注记录</td></tr>
			    ...
			    <tr height=20 align=center>
			     ...
			     <td style= "WIDTH: 40"><%=dbcInfo.rsGetString("times")%><BR></td>
			     <%
			     String strPageType=(String)session.getAttribute("pagetype");
			     if(intMarkIss==intIssueNum && strPageType.equals("man")){
			    	 out.print("<td width=30><a href=/servlet/useinfwagrem?play=lau&wagerid="+dbcInfo.rsGetString("id")+">删除</a></td>");
			     }
			     %>
			    </tr>
			   <%}%>
			 ...
			 <tr>
			  <td><ctl:page curpage="<%=psInfo.getCurrentPage()%>" maxpage="<%=psInfo.getMaxPage()%>" prevpage="<%=psInfo.getPrevPage()%>" nextpage="<%=psInfo.getNextPage()%>" servhref="servlet/servuserinfo?mark=lauwag&"></ctl:page>
			  </td>
			 </tr>
	       </table>
        </td>
