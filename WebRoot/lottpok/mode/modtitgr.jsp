<%@ page contentType="text/html; charset=gb2312"
    pageEncoding="gb2312" session="true"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />
<title>华彩在线-彩票投注网</title>
<link href="/pokercss/mainlaugh.css" rel="stylesheet" type="text/css" />

<script src="/pokerfunc/global.js"></script>
<script src="/pokerfunc/pokfunc/tytitle.js"></script>

<style type="text/css">
.modeready { background:url(/pkimages/pokerimage/modeready.gif) no-repeat center; text-align:center; cursor:hand}
.modechosen { background:url(/pkimages/pokerimage/modechosen.gif) no-repeat center; text-align:center; cursor:hand}

</style>
</head>
<body>

<table width=270 height=30 border="0">
  <tr>
    <td id="gr4" class="modechosen" width=40 height="30" style="color:white" onClick="chModeGr(this)">组4</td>
	<td id="gr6" class="modeready" width=40 height="30" onClick="chModeGr(this)">组6</td>
	<td id="gr12" class="modeready" width=40 height="30" onClick="chModeGr(this)">组12</td>
	<td id="gr24" class="modeready" width=40 height="30" onClick="chModeGr(this)">组24</td>
    <td align=center style="color:#808080"><br></td>
  </tr>
</table>
</body>
</html>
