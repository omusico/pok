 ...
 * @Version:1.0
 */
public class ExbTzInfoDaoImpl extends GenericEntityDAO<ExbTzInfo, String>
		implements IExbTzInfoDao {
	...
	public boolean jdbcSaveListInfo(List<ExbTzInfo> list) throws DAOException {
		...
		String insertSql = "insert into exb_tz_info (user_id, user_name, issue_num, play_type, type_name,play_mode, touzhu_num, touzhu_count, touzhu_times, touzhu_money, is_zhuitou, parent_issue_num, parent_id,is_valid, touzhu_time, ID,have_zt_flag,tz_seq)"
				+ " values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)";
		String insertSql2 = "insert into exb_tz_detail (id,p_id,issue_num,play_type,type_name,play_mode,num_detail,num_times,is_zhuitou)"
			+ " values (?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//					}
	...
	public List<ExbTzInfo> getUserTzInfo(String issueNum,String userId) {
		...
		String sql = " from ExbTzInfo t where 1=1 and t.isValid='1' ";
		...
		List<ExbTzInfo> list =  null;
		try{
			list = getList(sql);
		}catch(Exception e){
			logger.error("查询快乐十分用户投注信息错误:",e);
		}
		return list;
	}
	/**
	...
	public String getUserTzMaxSeq(String issueNum,String userId) {
		...
		String seq = "0";
		String sql = " select max(t.tzSeq) from ExbTzInfo t where 1=1 ";
		if(issueNum != null  && !issueNum.equals("")){
			sql = sql + " and t.issueNum = '" + issueNum + "' ";
		}
		if(userId != null  && !userId.equals("")){
		...
		try{
			List<ExbTzInfo> list  = getList(sql);
			...
			logger.error("查询快乐十分用户投注序号信息错误:",e);
		}
		return seq;
	}
	/**
	...
	public ExbTzInfo getUserTotalTzInfo(String issueNum,String userId) {
		...
		try{
			...
			logger.error("查询快乐十分指定用户总的投注信息错误:",e);
		}
		return info;
	}
}
