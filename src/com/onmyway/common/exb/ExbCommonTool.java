package com.onmyway.common.exb;
 ...
 * @Version:1.0
 */
public class ExbCommonTool {


	private IExbIssuePublicService exbIssuePublicService;
	...
	public String comparePlayAndServerTime(String gameType) {
		...
		// 得到相应的数据库设置的时间
		ExbBaseInfoConfig configinfo = exbIssuePublicService.getTodayConfig();
	 ...
	 * @return
	 */
	public String getExbNowIssueNumOnServer() {
		// 服务器当前时间
		GregorianCalendar calendar = new GregorianCalendar();
		Integer intYear = new Integer(calendar.get(Calendar.YEAR));
		Integer intMonth = new Integer(calendar.get(Calendar.MONTH) + 1);
		...
		IExbIssuePublicService iService = (IExbIssuePublicService)SpringContextUtil.getBean("exbIssuePublicService");
		ExbBaseInfoConfig configinfo = iService.getTodayConfig();
		beginDate = configinfo.getBeginDate();// 开始日期
		beginIssue = configinfo.getBeginIssue();// 开始期数
		beginHour = configinfo.getBeginHour();// 每天开始的小时
		beginMin = configinfo.getBeginMin();// 每天开始的分钟
	...
	public IExbIssuePublicService getExbIssuePublicService() {
		return exbIssuePublicService;
	}

	public void setExbIssuePublicService(
			IExbIssuePublicService exbIssuePublicService) {
		this.exbIssuePublicService = exbIssuePublicService;
	}
