package com.onmyway.common.ssc;
 ...
 * @Version:1.0
 */
public class SscCommonTool {

	private ISscIssuePublicService sscIssuePublicService;

	public ISscIssuePublicService getSscIssuePublicService() {
		return sscIssuePublicService;
	}

	public void setSscIssuePublicService(
			ISscIssuePublicService sscIssuePublicService) {
		this.sscIssuePublicService = sscIssuePublicService;
	}
	...
	public String comparePlayAndServerTime(String gameType) {
		...
		// 得到相应的数据库设置的时间
		SscBaseInfoConfig configinfo = sscIssuePublicService.getTodayConfig();
	 ...
	 * @return
	 */
	public String getSscNowIssueNumOnServer() {
		// 服务器当前时间
		GregorianCalendar calendar = new GregorianCalendar();
		Integer intYear = new Integer(calendar.get(Calendar.YEAR));
		Integer intMonth = new Integer(calendar.get(Calendar.MONTH) + 1);
		...
		ISscIssuePublicService iService = (ISscIssuePublicService)SpringContextUtil.getBean("sscIssuePublicService");
		SscBaseInfoConfig configinfo = iService.getTodayConfig();
		beginDate = configinfo.getBeginDate();// 开始日期
		beginIssue = configinfo.getBeginIssue();// 开始期数
		beginHour = configinfo.getBeginHour();// 每天开始的小时
		beginMin = configinfo.getBeginMin();// 每天开始的分钟
