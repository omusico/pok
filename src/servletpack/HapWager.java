...
//class login
public class HapWager extends HttpServlet
{
 ...
 throws IOException,ServletException{
	 ...
	 String typePageUrl="/lott/hap/play/"+req.getParameter("typepagemark")+".jsp";
	 String wagerInfoTable="hapwagerinfo";
	 
	 if(remInfo.getLoginUserName().equals("")){
		 req.setAttribute("wagrem","您还没有登录，请先登录!");
	 }else{
		 //在购买彩票时，判断是否已经超过销售时间
		 CompareBuyAndServerTime compare = new CompareBuyAndServerTime("hap");
	...
	//		 String strUWE=objUWE.useWagEnable(strUserName,strWagerNumTemp,strWagerTotal,strSelling);	
			 String strUWE=objUWE.useWagEnable(strMarkIss,strIssueNumber,strUserName,strWagerNumTemp,strWagerTotal,strSelling,"hap",strPlayType,strPlayMode);//判断是否可以进行购卖		 
	
			 if(!strUWE.equals("true")){
				 req.setAttribute("wagrem",strUWE);
			 }else{		
				 PokWagPub objPWP=new PokWagPub();
				 boolean wagerSuccess=objPWP.wager("hapwagerinfo", strUserName, strMarkIss, strIssueType, strIssueNumber, strPlayType, strPlayMode, strWagerTotal, strWagerNum, strSelling);
