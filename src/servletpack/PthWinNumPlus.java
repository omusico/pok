...
public class PthWinNumPlus extends HttpServlet
{	 
 public void doPost(HttpServletRequest req,HttpServletResponse res)
 throws IOException,ServletException
 {
	 HttpSession session = req.getSession(false);
	 String currIssueNum = new String(req.getParameter("pthissnum").getBytes("ISO8859_1"),"GB2312").trim();
	 ...
	 String sqlStrQue="select * from pthwinnum where issuenum='"+currIssueNum+"';";
	 winNumDBC.execQuery(sqlStrQue);
	 if(winNumDBC.rsNext()==false){
	   String sqlStr="insert into pthwinnum(issueday,issuenum,hundred,ten,one) values('" + issueDay + "','"+ currIssueNum + "','" + hundred + "','" + ten + "','" + one + "');";
	 ...
	 RequestDispatcher rd = sc.getRequestDispatcher("/servlet/loginmaninit?play=pth");
     rd.forward(req, res);
 }
 ...
 public void winUserToBase(String strIssueNum,String strHundred,String strTen,String strOne){
     ...
     winQueryDBC.execQuery("select * from pthwagerinfo where issuenum='"+strIssueNum+"' and valid='y' order by username;");//搜索出所有当期投注用户
     while(winQueryDBC.rsNext()){
    	 ...
    	 for(int i=0;i<wInfo.size();i++){
    		 ...
    		 String strSql = "insert into pthprizeinfo (username,issuenum,playtype,playmode,onewager,rule,wagernum,times,prize) values('" + strUserName + "','" + strIssNum + "','" + strPlayType + "','" + strPlayMode + "','" + strOneWager + "','" +strRule+"','"+ strWagerNum + "','" + strTimes + "','" + strPrize + "');";
    	 ...
    	 if(wInfo.size()>0){
    		 objPWP.invalid("lotteryuser","pthwagerinfo",strUserName,strMarkIss,strMarkIssSeq,strIssNum,strHasFol); 
    	 }else{
    		 objPWP.invalNoWin("pthwagerinfo",strUserName,strMarkIss,strMarkIssSeq,strIssNum);
    	 }
     }
 ...
 public String getPrize(String wagNum, String times,String rule,String[] arrWinFig){
	 ...
	 if(rule.equals("和值")){
		 ...
		 String[] arrRuleEng={"pthsingthr","pthgrthr","pthgrsix"};
		 GlobalMeth objGM=new GlobalMeth();
		 strRuleEng=objGM.tranCoorStr(arrRuleChin, arrRuleEng, rule);
	 }
	 pmDBC.execQuery("select * from pthprmoney where rule='"+strRuleEng+"';");
	 pmDBC.rsNext();
