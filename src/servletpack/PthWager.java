...
//class login
public class PthWager extends HttpServlet
{
 public void doPost(HttpServletRequest req,HttpServletResponse res)
 throws IOException,ServletException{
	 HttpSession session = req.getSession(false);
	 RemindInfo remInfo=(RemindInfo)session.getAttribute("loginmes");
	 String typePageUrl="/lott/pth/play/"+req.getParameter("typepagemark")+".jsp";
	 ...
	 String wagerInfoTable="pthwagerinfo";  
	 
	 if(remInfo.getLoginUserName().equals("")){
		 req.setAttribute("wagrem","您还没有登录，请先登录!");
	 }else{
		 //在购买彩票时，判断是否已经超过销售时间
		 CompareBuyAndServerTime compare = new CompareBuyAndServerTime("pth");
		 ...
		 if(issStat.equals("0")){
			 ...
			 //临时的，目前找不到在页面置这两个字段的值的地方，选用临时字段替代，以后找到了，再去掉
			 String tempStrIssueType= new String(req.getParameter("tempIssuetype").getBytes("ISO8859_1"),"GB2312").trim();
			 String tempStrIssueNumber = req.getParameter("tempGetissuenumber");
			 strIssueType= tempStrIssueType;
			 strIssueNumber = tempStrIssueNumber;
	...
	//		 String strUWE=objUWE.useWagEnable(strUserName,strWagerNumTemp,strWagerTotal,strSelling);
			 String strUWE=objUWE.useWagEnable(strMarkIss,strIssueNumber,strUserName,strWagerNumTemp,strWagerTotal,strSelling,"pth",strPlayType,strPlayMode);//判断是否可以进行购卖		 
	
			 if(!strUWE.equals("true")){
				 req.setAttribute("wagrem",strUWE);
			 }else{
				 PokWagPub objPWP=new PokWagPub();
				 boolean wagerSuccess=objPWP.wager("pthwagerinfo", strUserName, strMarkIss, strIssueType, strIssueNumber, strPlayType, strPlayMode, strWagerTotal, strWagerNum, strSelling);
